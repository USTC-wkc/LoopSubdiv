...
#ifdef __CUDACC__
__device__ __forceinline__ math::float4 from_cuda(const float4& v)
{
	return math::float4(v.x, v.y, v.z, v.w);
}
__device__ __forceinline__ math::float3 from_cuda(const float3& v)
{
	return math::float3(v.x, v.y, v.z);
}
__device__ __forceinline__ math::float2 from_cuda(const float2& v)
{
	return math::float2(v.x, v.y);
}
__device__ __forceinline__ math::int4 from_cuda(const int4& v)
{
	return math::int4(v.x, v.y, v.z, v.w);
}
__device__ __forceinline__ math::int3 from_cuda(const int3& v)
{
	return math::int3(v.x, v.y, v.z);
}
__device__ __forceinline__ math::int2 from_cuda(const int2& v)
{
	return math::int2(v.x, v.y);
}

__device__ __forceinline__ float4 to_cuda(const math::float4& v)
{
	return make_float4(v.x, v.y, v.z, v.w);
}
__device__ __forceinline__ float3 to_cuda(const math::float3& v)
{
	return make_float3(v.x, v.y, v.z);
}
__device__ __forceinline__ float2 to_cuda(const math::float2& v)
{
	return make_float2(v.x, v.y);
}
__device__ __forceinline__ int4 to_cuda(const math::int4& v)
{
	return make_int4(v.x, v.y, v.z, v.w);
}
__device__ __forceinline__ int3 to_cuda(const math::int3& v)
{
	return make_int3(v.x, v.y, v.z);
}
__device__ __forceinline__ int2 to_cuda(const math::int2& v)
{

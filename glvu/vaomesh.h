...
struct GLMesh
{
    ...
    std::tuple<int, int,  Eigen::Vector4f> moveCurrentVertexTuple(int x, int y, const int* vp)
    {
        if (actVertex < 0 || actVertex >= nVertex) return std::make_tuple(-1, 0, Eigen::Vector4f{});
        ...
        return std::make_tuple(0, actVertex, x1);
    }
